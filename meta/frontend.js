...
function signMessage() { 
	...
	ethereum.request({ method: 'eth_accounts' }).then(function(result) {
		...
		//return result[0];
		localStorage.setItem('user_addreess',result[0])
		document.getElementById('loginTexts').style.display = "none";
		document.getElementById('useraddress').innerHTML = result[0];
		document.getElementById('useraddress').style.display = "block";
	//	document.getElementById('texts').innerHTML = 'Logout';
		return socket.emit('getnonce', { useraddress: result[0] })
	});
	...
	//socket.emit('getnonce', { useraddress: getUserAddress })
}
socket.on('returnednonce', function (nonce) {
	var adds = ethereum.request({ method: 'eth_accounts' }).then(function(result) {
		return result[0];
	});
  console.log(nonce)
  nonce = "{Your Site} login request. Nonce: " + nonce
    web3.eth.personal.sign(nonce, adds, function (err, res) {
  //web3.personal.sign(nonce,  ethereum.request({ method: 'coinbase' }), function (err, res) {
    let signature = res
    socket.emit('signature', { signature: signature, nonce: nonce, useraddress: adds })
  })
})
...
socket.on('loggedin', function () {
 ...
 var adds = ethereum.request({ method: 'eth_accounts' }).then(function(result) {
  ...
  adds != null ? $('#loginText').html("You are logged in as " + localStorage.getItem('address')) : $('#loginText').html("Please login with MetaMask.")
  keepAlive()
})
  ...
  function handleAccountsChanged(accounts) {
    if (accounts.length === 0) {
      // MetaMask is locked or the user has not connected any accounts
        document.getElementById('loginTexts').style.display = "block";
		//document.getElementById('useraddress').innerHTML = result[0];
		document.getElementById('useraddress').style.display = "none";
      console.log("Please connect to MetaMask.")
      localStorage.removeItem('user_addreess')
